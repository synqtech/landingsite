class RoiSurveysController < ApplicationController
  ...
  def new
    ...
    unless params['d'] == 'false'
      @roi_survey.digi = true
    end
  end
  ...
  private
    ...
    def roi_survey_params
      params.require(:roi_survey).permit(:store_sq_ft, :haba, :cctv, :walking_svc, :third_party_inventory, :civil_claim_won, :price_reduction_markdown, :existing_eas, :eas_effective, :lpo_hours, :eas_tags_per_month, :total_weekly_sales, :meat_sales_percent, :num_pos_terminals, :num_exits, :exit_width_ft, :digi)
    end
end
